...
> 使用 ngram 匹配做节点级来源校验,校验单元是 tree node。
...
例:`result-fixtures/topic-only-cet/provenance-audit.json`
...
- audit.summary.high_ratio < 0.60 → ⚠️ 触发"整篇置信度降级"
...
| 维度 | claim-level audit | knowledge-map |
|---|---|---|
...
3. **跨语言匹配**:用户 material 是中文教材但 LLM 用英文术语标 title 时,ngram 命中率会偏低;必要时应要求用户确认术语对应关系
